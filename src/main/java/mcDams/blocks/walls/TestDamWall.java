...
private boolean checkBlockMatch(World world, int x, int y, int z){
	...
	//if (testblock.getUnlocalizedName().equals(this.getUnlocalizedName())){
	...
	if (testblock.getUnlocalizedName() == McDams.testValveBlock.getUnlocalizedName()){
		return true;
	}
	return false;
	...
	if (checkBlockMatch(world,x,y-1,z)){
		...
		if (checkBlockMatch(world,x-1,y,z)){
			...
			if (checkBlockMatch(world,x-2,y,z)){
				// there is a valid block x-2 - I am top right	- top row valid
				topRow = true;
				topPossition = 2;
			}			
			
			if (checkBlockMatch(world,x+1,y,z)){
				// there is a valid block x+1 - I am top middle - top row valid
				topRow = true;
				topPossition = 1;
			}
		...
		if (checkBlockMatch(world,x+1,y,z)){
			...
			if (checkBlockMatch(world,x+2,y,z)){
				// there is a valid block x+2 - I am top left	- top row valid
				topRow = true;
				topPossition = 0;
			}			
